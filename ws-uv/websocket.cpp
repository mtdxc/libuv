    ...
    namespace websocket {
        ...
        void WebSocket::decode_fragments()
        {
            for (size_t i = fragment_offset;;) {
                auto len = decode_frame((char*)fragment_buffer.data() + i, fragment_buffer.size() - i);
                if (!len) {
                    fragment_offset = i;
                    break;
                }
        ...
        void WebSocket::send(const char* buf, size_t length, WebSocketOpcode op)
        {
            ...
            std::vector<char> frame(length + 14, 0);
            size_t header_length = WebSocketCreateFrameHeader(frame.data(), frame.size(),
        ...
        void WebSocket::send_raw(const char * str, size_t len)
        {
            auto write_req = new uv_write_t{};
            write_req->data = this;

            uv_buf_t bufs[1];
            bufs[0].base = (char*)str;
            bufs[0].len = len;
        ...
        WebSocketFrame WebSocketExtractFrameHeader(const char * source, size_t len)
        {
            ...
            if (res.is_masked) {
                ...
                res.mask = htonl(*(long*)(source + off));
				off += 4;
            }
...
#else
			char buff[41] = {0};
			shacalc(key.c_str(), buff);
			send_http_response(buff);
#endif
